...
(function Game() {
    ...
    var undoButton = document.getElementById('undo-move');
    undoButton.addEventListener('click', undoMove, false);
    ...
    var resetGameHandler = function() 
    {
        ...
        //xturnDisplay.className = currentContext;
    ...
    var undoMove = function() {
        ...
        // Decrement the turns counter and switch the current context
        turns--;
        currentContext = computeContext();
        
        // Remove the most recent move from the board array
        var lastMove = moves.pop();
        var row = lastMove[0];
        var col = lastMove[1];
        board[row][col] = "";
        
        // Clear the corresponding cell on the game board
        var cell = boxes[row * 3 + col];
        cell.className = '';
        cell.innerHTML = '';
        
        // Add back the click event listener to the cell
        cell.addEventListener('click', clickHandler, false);
        
        // Hide the game messages if they were previously shown
        gameMessages.className = '';
      };
